const CollectionTable = require('../../../../../models/kw_recharges');
/**********************************************************************
...
exports.select = async function (options) {
    try {
      ...
      if(type === 'count'){
        ...
        return await CollectionTable.find(condition).countDocuments(condition);
      } else if(type === 'single'){
...
/**********************************************************************
 * Function Name    :   select
 * Purposs          :   This function is used for select from orders table
 * Created By       :   Dilip Halder
      ...
      .populate([
        {
          path   : "created_by",
          model  : "kw_users",
          select : "_id users_type users_name last_name users_email pos_number country_code users_mobile users_seq_id bind_person_name store_name status commission_percentage "
        }
      ]);
    } else {
      return await CollectionTable.find(condition).select(select)
      .populate([
        {
          path   : "created_by",
          model  : "kw_users",
...
exports.rechargeAggregate = async function (options) {
  try {
      return await CollectionTable.aggregate(options);
  } catch (error) {
      return Promise.reject(error);
  }
}//End of Function