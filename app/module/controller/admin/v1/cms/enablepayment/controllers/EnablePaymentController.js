...
const Services = require("../../../../../../services/admin/v1/cms/EnablePaymentCMSServices");
...
exports.add = async function (req, res) {
    ...
    try {
        const { title_stripe,stripe,title_telr,telr,title_noon,noon } = req.body;
        const counter = await counterService.getSequence('kw_paymentmode');
        const options = {
            section_id  : counter.seq,
            title_stripe: title_stripe,
            stripe      : stripe,
            title_telr  : title_telr,
            telr        : telr,
            title_noon  : title_noon,
            noon        : noon,
            status      : "A",
...
exports.update = async function (req, res) {
    try {
        ...
        const {  id, status, title_stripe,stripe,title_telr,telr,title_noon,noon } = req.body;
        ...
        const options = {
            ...
            data: {
                ...(title_stripe ? { title_stripe: title_stripe } : ''),
                ...(stripe ? { stripe: stripe } : ''),
                ...(title_telr ? { title_telr: title_telr } : ''),
                ...(telr ? { telr: telr } : ''),
                ...(title_noon ? { title_noon: title_noon } : ''),
                ...(noon ? { noon: noon } : ''),
