...
const LoadbalanceArchiveServices = require("../../../../../services/front/v1/loadbalanceArchiveServices");
 /*********************************************************************************
 * Function Name    :   List
 ...
 ********************************************************************************/
 exports.list = async function (req, res) {
    try {
        ...
        const result = await LoadbalanceServices.select(listWhere);
 ...
 ********************************************************************************/
 exports.updateFormatedDate = async function (req, res) {
    try {
        console.log('Update loadbalance formated date');
        let listWhere = {
        ...
        result.map(async (item)=>{
            ...
            await LoadbalanceServices.updateData(options);
        })
        return true;
        return response.sendResponse(res, response.build('SUCCESS', { result }));
    } catch (error) {
 ...
 ********************************************************************************/
 exports.movetoarchive = async function (req, res) {
    try {
        ...
        const orderData = await LoadbalanceServices.select(listWhere);
        if(orderData && orderData.length > 0){
            const IdList = orderData.map((item)=>item._id);
            const archiveInsertResult = await LoadbalanceArchiveServices.create(orderData);
            if (archiveInsertResult && archiveInsertResult.length === orderData.length) {
                await LoadbalanceServices.deleteData({ _id: { $in: IdList } });
